...
Интерьер Radiocentr: `maps/radio.txt`, VW **8**, interior **0** (кастомный маппинг в небе). Спавн сотрудников: `1429.54, 1071.88, 1058.79`.
...
Транспорт Radiocentr (News Chopper 488 на крыше, Newsvan 582 ×4 у HQ) — цвета **1/152**, только `org_id = 8`. Чужому: двери закрыты. `vehicles/radio.ts`.

`/ad` — любой, диалог **37**, **$500** наличными. Мут блокирует. Одно объявление в очереди. Сотрудникам: `Postupilo novoe ob'yavlenie ot Name[id]. Vvedite /edit.` `/edit` (диалоги **38–39**) только у стола `1424.46, 1056.62` (радиус 5 м, VW **8**) или в транспорте Radiocentr. Принять — в эфир не раньше чем через 3 мин, пауза 3 мин между объявлениями. Отклонить — причина игроку и всем сотрудникам. Эфир: `LS | tekst. | Otpravil(a) Name[id]` зелёным, вторая строка темнее. `commands/ads.ts`.

Шлагбаум Radiocentr (модель **968**, `ry` 90→0) у `1739.45, -1309.92, 13.50`. Crouch. Только `org_id = 8`. Стойка 966 уже в `maps/radio.txt`. `org/radio.ts` + `org/gates.ts`.
...
| `/ad` | подать объявление ($500 наличными), очередь Radiocentr |
| `/edit` | сотрудник Radiocentr правит объявление в офисе или в своём транспорте |
...
| 36 | Radiocentr: офис / улица / крыша |
...
| Интерьер Radiocentr | `maps/radio.txt` |
| Входы Radiocentr | `org/radio-doors.ts` |
| Фотоаппарат Radiocentr | `org/radio-locker.ts` |
| Шлагбаум Radiocentr | `org/radio.ts`, `org/gates.ts` |
